const PrescriptionQueries = {
    ...
    getFirstTimers:"SELECT email,first_timer,first_name,last_name,drug_name FROM prescriptions INNER JOIN users ON users.id = prescriptions.user_id WHERE prescriptions.status = 'active' AND first_timer =$1",
    getSecondTimers:"SELECT email,second_timer,first_name,last_name,drug_name FROM prescriptions INNER JOIN users ON users.id = prescriptions.user_id WHERE prescriptions.status = 'active' AND second_timer =$1",
    getThirdTimers:"SELECT email,third_timer,first_name,last_name,drug_name FROM prescriptions INNER JOIN users ON users.id = prescriptions.user_id WHERE prescriptions.status = 'active' AND third_timer =$1",
